...
const getConfession = async hash => gql('       \
    query Q($hash: String) {                    \
        confession(hash: $hash) { title, hash } \
    }',
    { hash }
)
      .then(d => d.confession);
...
const getConfessionWithMessage = async id => gql('                         \
    mutation Q($id: String) {                                              \
        confessionWithMessage(id: $id) { title, hash, message, timestamp } \
    }',
    { id }
)
      .then(d => d.confessionWithMessage);


const addConfession = async (title, message) => gql('          \
    mutation M($title: String, $message: String) {             \
        addConfession(title: $title, message: $message) { id } \
    }',
    { title, message }
 )
      .then(d => d.addConfession);


const previewHash = async (title, message) => gql('              \
    mutation M($title: String, $message: String) {               \
        addConfession(title: $title, message: $message) { hash } \
    }',
    { title, message }
    )
      .then(d => d.addConfession);
...
const show = async confession => {
    if (confession) {
        ...
        preview.querySelector('.date').textContent = `${confession.timestamp}` || '<unknown date>';
        ...
        preview.querySelector('.how-to-verify').textContent = `sha256(${JSON.stringify(confession.message || '<message>')})`;
    } else {
...
publish.onclick = async () => {  // publish a confession
    title.disabled = true;
